	...
	$("#search").click(function(){
		try {
			...
			navigator.geolocation.getCurrentPosition(function(position){
				...
				"&name=" + $("#searchbox").val() +
				"&sensor=false&key=AIzaSyC4vCfT_Knq1SGuNMahZqyrmZFiTuBsdlY";
				$.getJSON(url, function(data){
					...
					try {
						$(data.results).each(function(index, entry){
							...
							entry.icon + "\" class=\"ui-li-icon\"></img><h3>&nbsp;" + entry.name + "</h3><p><strong>&nbsp;vicinity:" + entry.vicinity + "</strong></p></a>";
...
function fetchDetails(entry){
	...
	$.mobile.showPageLoadingMsg();
	var detailsUrl = "https://maps.googleapis.com/maps/api/place/details/json?reference=" + entry.reference + "&sensor=true&key=AIzaSyD8eeHlKL3KoHSGFbP9UFgZn7GNjpEJABY";
	$("#name").html("");
	$("#address").html("");
	$("#phone").html("");
	$("#rating").html("");
			...
			tx.executeSql('SELECT * FROM Favorite', [], function (tx, results) {
				...
				if (results != null && results.rows != null) {
					for (var index = 0; index < results.rows.length; index++) {
						...
						var htmlData = "<a href=\"#details\" id=\"" + entry.reference + "\"><img src=\"" + entry.icon + "\" class=\"ui-li-icon\"></img><h3>&nbsp;" + entry.name + 
